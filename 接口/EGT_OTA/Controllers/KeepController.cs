    ...
    public class KeepController : BaseController
    {
        ...
        public ActionResult All()
        {
            try
            {
                ...
                var articles = new SubSonic.Query.Select(Repository.GetProvider(), "ID", "Title", "TypeID", "Cover", "Views", "Goods", "Keeps", "Comments", "CreateUserID", "CreateDate", "ArticlePower", "ArticlePowerPwd", "Tag").From<Article>().Where("ID").In(list.Select(x => x.ArticleID).ToArray()).OrderDesc(new string[] { "Tag", "ID" }).ExecuteTypedList<Article>();
