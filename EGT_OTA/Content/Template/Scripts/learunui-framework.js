...
function delConfig(url, parm, count) {
    ...
    confirmDialog("提示", "注：您确定要删除 " + count + " 条记录？", function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
            window.setTimeout(function () {
                AjaxJson(url, parm, function (data) {
                    ...
                    tipDialog(message, 3, data.result);
                    if (data.result > 0) {
                        windowload();
                    }
                });
...
function doConfig(name, url, parm, count) {
    if (count == undefined) {
        count = 1;
    }
    confirmDialog("提示", "注：您确定要" + name + count + " 条记录？", function (r) {
        if (r) {
            Loading(true, "正在" + name + "数据...");
            window.setTimeout(function () {
                AjaxJson(url, parm, function (data) {
                    var message = "失败";
                    if (data.result > 0) {
        ...
        $.each(JsonData, function (i) {
            if (JsonData[i].ParentId == '0' && JsonData[i].Category == '1') {
                ...
                if (toolbar_Children_Count > 0) {
                    ...
                    toolbar_html += "<div style=\"float: left;\"><div class=\"icon\"><img src=\"/Content/Template/Images/" + JsonData[i].Icon + "\" /></div><div class=\"text\">" + JsonData[i].FullName + "</div></div>";
                    ...
                    toolbar_html += "<a id=\"" + JsonData[i].Code + "\" ids=\"" + JsonData[i].ButtonId + "\" " + title + " onclick=\"" + JsonData[i].JsEvent + "\" class=\"tools_btn\"><span><b style=\"background: url('/Content/Template/Images/" + JsonData[i].Icon + "') 50% 4px no-repeat;\">" + JsonData[i].FullName + "</b></span></a>";
                }
    ...
    function Toolbar_Children(ButtonId, Data) {
        ...
        $.each(Data, function (i) {
            if (Data[i].ParentId == ButtonId) {
                ...
                _toolbar_Children += "<li id=\"" + Data[i].Code + "\" ids=\"" + Data[i].ButtonId + "\" " + title + " onclick=\"" + Data[i].JsEvent + "\"><img src=\"/Content/Template/Images/" + Data[i].Icon + "\" />" + Data[i].FullName + "</li>";
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace, IsVisitorModule) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','true')\"><a><i class='fa " + img + "' width='16' height='16'></i>" + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");
        } else {
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class=\"selected\" onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','false')\"><a><i class='fa " + img + "' width='16' height='16'></i>" + name + "</a></li>");
        }
