...
namespace EGT_OTA.Controllers
{
    ...
    public class UserInfoController : BaseController
    {
        ...
        public ActionResult Delete()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var list = new SubSonic.Query.Select(Repository.GetProvider()).From<Role>().And("ID").In(array).ExecuteTypedList<UserInfo>();
                    list.ForEach(x =>
                    {
                        x.Status = status;
                    });
        ...
        public ActionResult Audit()
        {
            ...
            if ((status == Enum_Status.Approved && !CurrentUser.HasPower("21-5")) || (status == Enum_Status.Audit && !CurrentUser.HasPower("21-6")))
            {
                return Json(new { result = result, message = "您不是管理员或者没有管理的权限" }, JsonRequestBehavior.AllowGet);
            }
            var ids = ZNRequest.GetString("ids");
