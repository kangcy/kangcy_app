...
namespace EGT_OTA.Models
{
    ...
    public class Upload2Config
    {
        ...
        /// 单体类访问实体类
        /// </summary>
        public static Upload2Config Instance
        {
            get
            {
                if (HttpContext.Current.Cache["Upload2Config"] == null)
                {
                    lock (lockObject)
                    {
                        if (HttpContext.Current.Cache["Upload2Config"] == null)
                        {
                            Upload2Config config = new Upload2Config();
                            ///添加到缓存
                            HttpContext.Current.Cache.Insert("Upload2Config", config, new CacheDependency(configFilePath), Cache.NoAbsoluteExpiration, TimeSpan.FromHours(24));
                            return config;
                        }
                    }
                }
                ///从缓存中获取
                return HttpContext.Current.Cache["Upload2Config"] as Upload2Config;
            }
        }
