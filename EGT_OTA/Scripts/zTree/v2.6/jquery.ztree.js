...
//加载样式文件
$(function() {
    if ($("head link[zTree]").length == 0) {
        $("head:first").append("<link rel='stylesheet' type='text/css' href='/Scripts/zTree/zTreeStyle/zTreeStyle.css' zTree='1' />");
    }
});
...
(function($) {
    ...
    $.fn.zTree = function(zTreeSetting, zTreeNodes) {
        ...
        $(this).addClass("tree_" + setting.theme);
    ...
    function appendTreeNodes(setting, level, treeNodes, parentNode) {
        ...
        for (var i = 0, l = treeNodes.length; i < l; i++) {
            ...
            //[2011-09-13]edit by laone
            if (node.customNode) {
                html.push("<a id='", node.tId, IDMark_A, "' treeNode", IDMark_A, " onclick='", (node.click || ''), "' customNode='1' class='", node.customNode.cssClass, "' ",
                ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='", makeNodeTarget(node), "' style='", fontStyle.join(''),
				"'><span id='", node.tId, IDMark_Span, "'",
                ((setting.titleClickOpenChildren || node.titleClickOpenChildren) ? " openChildren='1'" : ""),
				">", node[setting.nameCol].replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'), "</span></a>");

                html.push("<ul id='" + node.tId + IDMark_Ul + "' style='width:" + (node.customNode.subWidth ? node.customNode.subWidth + "px" : "100%") + ";height:" + (node.customNode.subHeight ? node.customNode.subHeight : "300") + "px;overflow:auto;display:", (node.open ? "block" : "none"), "'>");
            } else {//edit end
                html.push("<button type='button' id='", node.tId, IDMark_Switch,
                ...
                ((setting.titleClickOpenChildren || node.titleClickOpenChildren) ? " openChildren='1'" : ""),
				">", node[setting.nameCol].replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'), "</span></a>");
                html.push("<ul id='", node.tId, IDMark_Ul, "' class='", makeUlLineClass(setting, node), "' style='display:", (node.open ? "block" : "none"), "'>");
            }
    ...
    var handler = {
        ...
        onSwitchNode: function(event) {
            ...
            //[2011-09-09]edit by laone
            if (setting.closeSameLevel && !treeNode.open) {//打开菜单时是否关闭同级的菜单
                //expandCollapseSonNode(setting, treeNode.parentNode, false); //$("#" + event.data.treeObjId).find("li[level='" + treeNode.level + "]").hide();
                var treeNodes = (treeNode.parentNode) ? treeNode.parentNode[setting.nodesCol] : setting.root[setting.nodesCol];
                //针对动画进行优化,一般来说只有在第一层的时候，才进行动画效果
                if (treeNodes) {
                    for (var son = 0; son < treeNodes.length; son++) {
                        if (treeNodes[son]) expandCollapseSonNode(setting, treeNodes[son], false, false);
                    }
                }
            }
            //edit end
        ...
        onClickNode: function(event) {
            ...
            //[2011=09-13] edit by laone
            if (setting.titleClickOpenChildren) {
                handler.onSwitchNode(event);
            }
            //edit end
        },
    ...
    function addTreeDom(setting, treeNode) {
        ...
        if (treeNode.customNode) {//自定义鼠标经过节点时的样式
            $("#" + treeNode.tId).children("a[customNode]").addClass(treeNode.customNode.hover);
        }
    }
    //删除zTree的按钮控件
    function removeTreeDom(setting, treeNode) {
        ...
        if (treeNode.customNode) {
            $("#" + treeNode.tId).children("a[customNode]").removeClass(treeNode.customNode.hover);
        }
    }
    //删除 编辑、删除按钮
    ...
    function selectNode(setting, treeNode) {
        ...
        if (setting.editable && treeNode.editNameStatus) {
            ...
            if (treeNode.customNode) {
                $("#" + setting.treeObjId).find("a." + treeNode.customNode.current).removeClass(treeNode.customNode.current);
                $("#" + treeNode.tId + IDMark_A).addClass(treeNode.customNode.current);
            }
            else {
